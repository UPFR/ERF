...
def getCommunity_BySentences(AdjacentMatrix,tfidf_matrixt,MIN_SIMILARITY,MAX_SIMILARITY):
    ...
    while(1):
        ...
        if seed !=-1:
            ...
            Propagate(AdjacentMatrix,tfidf_matrixt,new_community,centroid,left_nodes,MIN_SIMILARITY)
...
#迭代过程
def Propagate(AdjacentMatrix,tfidf_matrixt,coms,centroid,left_nodes,MIN_SIMILARITY):
    #获取当前社区的所有邻居节点以及邻居节点到当前社区质心的距离
    nel, nelf = getCommunityNeighbours(AdjacentMatrix,tfidf_matrixt,coms,centroid,left_nodes)
    #所有的邻居节点与质心的距离都小于阈值
    if len(nelf)>0 and max(nelf) >= MIN_SIMILARITY:
        ...
        #去掉离质心距离小于阈值的所有节点
        removeCommunityMenmbers(tfidf_matrixt,coms,centroid,MIN_SIMILARITY)
        #重新计算质心
        centroid=getCentroid(tfidf_matrixt,coms)
        #以新的质心继续迭代
        Propagate(AdjacentMatrix,tfidf_matrixt,coms,centroid,left_nodes,MIN_SIMILARITY)
    else:
        return coms
...
#去除远离质心的节点
##################################################################################           
def removeCommunityMenmbers(tfidf_matrixt,coms,centroid,MIN_SIMILARITY):
    to_be_removed_list=[]
...
##################################################################################
def getCommunityNeighbours(AdjacentMatrix,tfidf_matrixt,coms,centroid,left_nodes):
    #先获取当前社区的所有邻居节点
    nel = getAllneighbours(AdjacentMatrix,coms,left_nodes)
    nelf= []
